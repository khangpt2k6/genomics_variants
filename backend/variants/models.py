...
class CancerTrendPrediction(models.Model):
    """
    AI-generated predictions for cancer variant trends.

    This model stores trend predictions generated by LLM analysis,
    including future variant discovery patterns and risk assessments.
    """
    ...
    prediction_id = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="Unique identifier for the prediction"
    )
    prediction_date = models.DateTimeField(
        auto_now_add=True,
        help_text="Date when prediction was generated"
    )
    prediction_horizon_days = models.PositiveIntegerField(
        default=30,
        help_text="Number of days the prediction covers"
    )
    ...
    predicted_variant_counts = models.JSONField(
        default=list,
        blank=True,
        help_text="Predicted daily variant counts as JSON array"
    )
    prediction_dates = models.JSONField(
        default=list,
        blank=True,
        help_text="Corresponding dates for predictions as JSON array"
    )
    confidence_intervals = models.JSONField(
        default=list,
        blank=True,
        help_text="Confidence intervals for predictions as JSON array"
    )
    ...
    key_trends = models.JSONField(
        default=list,
        blank=True,
        help_text="Key trends identified by LLM analysis"
    )
    significant_genes = models.JSONField(
        default=list,
        blank=True,
        help_text="Genes showing significant trends"
    )
    clinical_patterns = models.JSONField(
        default=list,
        blank=True,
        help_text="Clinical significance patterns identified"
    )
    drug_implications = models.JSONField(
        default=list,
        blank=True,
        help_text="Drug response implications"
    )
    risk_assessment = models.TextField(
        blank=True,
        help_text="Overall risk assessment from LLM analysis"
    )
    recommendations = models.JSONField(
        default=list,
        blank=True,
        help_text="Recommendations for monitoring and action"
    )
    ...
    trend_chart_data = models.JSONField(
        default=dict,
        blank=True,
        help_text="Chart data for trend visualization"
    )
    gene_chart_data = models.JSONField(
        default=dict,
        blank=True,
        help_text="Chart data for gene frequency visualization"
    )
    ...
    data_source_period_days = models.PositiveIntegerField(
        default=180,
        help_text="Number of days of historical data used for prediction"
    )
    total_variants_analyzed = models.PositiveIntegerField(
        default=0,
        help_text="Total number of variants analyzed for this prediction"
    )
    ...
    def __str__(self):
        return f"Trend Prediction {self.prediction_id} - {self.trend_direction}"

    def get_prediction_summary(self):
        """Return a summary of the prediction results."""
        return {
            'direction': self.trend_direction,
            'confidence': f"{self.confidence_score:.2%}",
            'horizon': f"{self.prediction_horizon_days} days",
            'key_trends_count': len(self.key_trends),
            'significant_genes_count': len(self.significant_genes)
        }
    ...
    def get_predicted_range(self):
        """Get the date range covered by this prediction."""
        if not self.prediction_dates:
            return None

        dates = [datetime.fromisoformat(d) for d in self.prediction_dates if d]
        if not dates:
            return None
    ...
    def get_risk_level(self):
        """Determine risk level based on trend direction and confidence."""
        if self.trend_direction == 'increasing' and self.confidence_score > 0.7:
            return 'high'
